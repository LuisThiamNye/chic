...
(do
  (def --tree-code
    (util/quoted
     (ui3/fnlet-widget
      (fn ui-linear-treeview [^Rect rect ^Rect visible-rect ^float scale mouse-pos
        ...
        (let [{:keys [first-visible-idx last-visible-idxe content-x content-y]}
          ...
          {:draw
           (fn [cnv]
             ;; (debug/println-main "ltree" (ui3/get-changed-input-syms))
             (when hover-idx
